...
class thread extends Abstract2chAPI {
    ...
    public function CreateArray ($conte) {
        ...
        $thread['Status'] = 'ok';
        $thread['bord'] = $this->getBordName($this->bord);
    ...
    private function getBordName ($key) {
        $json = @file_get_contents('../../classes/2ch/BordList.json');
    ...
    private function getDatURL ($id) {
        $url = 'http://'.$this->sub_domain.'.'.$this->domain.'/'.$this->bord.'/dat/'.$id.'.dat';
        return $url;
    }

    public function getURL () {
        $url = 'http://'.$this->sub_domain.'.'.$this->domain.'/'.$this->bord.'/subject.txt';
        return $url;
    }
